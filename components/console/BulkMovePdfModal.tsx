...
export default function BulkMovePdfModal({
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={`Move ${selectedCount} PDF${selectedCount !== 1 ? 's' : ''}`}>
      <div className="space-y-4">
        <p className="text-sm text-gray-600">Select a folder to move the selected PDFs to:</p>
        ...
        <div className="max-h-96 overflow-y-auto border border-gray-200 rounded-lg">
          <div className="space-y-1 p-2">
            {allFolders.map((folder) => (
              ...
              >
                <FolderOpen weight="regular" className="w-4 h-4 flex-shrink-0" />
