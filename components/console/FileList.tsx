  ...
  const moveFolder = async (targetFolderId: string | null) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setMovedFolders(
          Object.fromEntries(
            Object.entries(movedFolders).filter(
              ([key]) => key !== moveModal.id,
            ),
          ),
        );
        showToast(
      ...
      setMovedFolders(
        Object.fromEntries(
          Object.entries(movedFolders).filter(([key]) => key !== moveModal.id),
        ),
      );
      showToast(
        "error",
  ...
  const movePdf = async (targetFolderId: string) => {
    ...
    try {
      ...
      if (!response.ok) {
        setMovedPdfs(
          Object.fromEntries(
            Object.entries(movedPdfs).filter(
              ([key]) => key !== movePdfModal.id,
            ),
          ),
        );
        showToast(
      ...
      setMovedPdfs(
        Object.fromEntries(
          Object.entries(movedPdfs).filter(([key]) => key !== movePdfModal.id),
        ),
      );
      showToast(
        "error",
                ...
                {isSelectMode && (
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-600 uppercase tracking-wider">
                    <input
                      ...
                      checked={
                        summaryFiles.length > 0 &&
                        summaryFiles.every((file) => selectedPdfs.has(file.id))
                          ? true
                          : false
                      }
                      onChange={() => {
                  ...
                  file.summaryData && expandedSummaries.has(file.id)
                    ? file.summaryData.map((association, index) => {
                        ...
                        const hasAssociationNotice =
                          association.notice &&
                          association.notice.trim().length > 0;
                ...
                {isSelectMode && (
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-600 uppercase tracking-wider">
                    <input
                      ...
                      checked={
                        regularFiles.length > 0 &&
                        regularFiles.every((file) => selectedPdfs.has(file.id))
                          ? true
                          : false
                      }
                      onChange={() => {
